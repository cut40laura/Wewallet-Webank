...
- **账本状态在客户端**（`videoCall.businessLedger`），贯穿整通；用**版本号**防乱序覆盖（§4.3）。
...
businessClaimEvents: []  // [ { field, value, raw, ts, utterance_seq, changed:Boolean } ]
```

- `openVideoCall` 时把两者都重置（账本清空、events 清空）。
- 每次抽取接口返回后：按版本号决定是否采纳（§4.3）；采纳则覆盖账本、并把本句涉及的每个业务主张 append 进 `businessClaimEvents`（`changed=true` 表示与账本旧值不同）。
...
```json
{
  ...
  "claim_events": [
    { "field": "loan_total", "value": 1000, "unit": "万", "raw": "要1000万", "changed": true }
  ],
...
字段说明：
- `seq`：原样回传请求里的 `seq`，客户端据此丢弃乱序的旧响应（§4.3）。
...
- `updated_ledger`：把本句抽到的业务主张并入账本后的结果（已归一化）；本句未提及的字段**原样保留**，不清空。
- `claim_events`：本句涉及的业务主张逐条（含是否 `changed`），客户端 append 到 `businessClaimEvents`。
...
**抽取规则（关键，避免误报，点4 & 点8）：**
- **"未提及" ≠ 变化**：本句没说到某字段，**不算矛盾**，账本保留旧值。只有用户**明确改值**（500→1000）或**明确移除/归零**（"工资不用了/全投厂房设备/工资 0"）才算 `changed`/矛盾。
...
### 4.3 客户端接入（`videoRunBusinessCheck`）+ 乱序防护

仿照现有 `videoRunContradictionCheck`，但**不依赖 `memoryText`**（无档案也要做自我一致性）：

- 在 `videoScheduleContradictionCheck` 去抖回调里**并行**调用。
- **乱序防护（点2）**：维护 `videoCall.businessSeq`（每次派发自增）。请求带上 `seq`；响应回来时只有 `resp.seq === videoCall.businessSeq` 才采纳 `updated_ledger`/events，否则丢弃（旧响应后到不回滚新账本）。另保留单 in-flight 守卫 `businessCheckInFlight`（仿现有 `checkInFlight`）。
- 采纳后：覆盖 `businessLedger`、append `claim_events` 到 `businessClaimEvents`、`contradictions` 逐条进业务状态机（§4.4）。
- `is_business=false`：仍可更新账本，但不触发核验/nudge。
...
- **实时语音通道**：经 `videoCall.client.sendContext(...)` 注入实时 WebSocket 上下文；代理已改为"句末(TTSEnded)再喂"，steer 下一句、不打断。
...
- **审计快照（点6）**：挂断时写入
  ...
  - `risk.business_claim_events`：完整事件流水（每条 `field/value/raw/ts/seq/changed`），人工复核可据此一眼看清"报过哪些数、变过几次"。

## 5. 数据流（端到端举例）

1. 第 3 轮用户："想借500万，300万厂房、100万设备、100万工资。" → 抽取写入 `loan_total=500万 / allocations={厂房:300,设备:100,工资:100}`；events 各记一条 `changed:true`（首次出现）；无矛盾。
2. AI 正常介绍流程。
3. 第 25 轮用户（装生气）："早说了要1000万！800万厂房、200万设备！你怎么还不懂！" → 对比账本：`loan_total 500→1000`、`厂房 300→800`、`设备 100→200` 三处 `changed`；**工资本句没提 → 不判矛盾、账本保留 100**（点4）。`is_business=true`、`emotional_pressure=true`，返回 3 条规范形 contradictions。
...
- `ui/static/chat.js`：
  - `videoCall.businessLedger` / `businessClaimEvents` / `businessIntegrity` / `businessSeq` / `businessCheckInFlight` 字段与 `openVideoCall` 重置；
  - `videoRunBusinessCheck`（含 seq 乱序防护）；`videoScheduleContradictionCheck` 接入；
  - 业务升级状态机；
